...
public class Graph
{
    ...
    public Graph Transpose(){
        Graph transposedGraph = new Graph(4,true);
        for (int i =0; i<nodes.size();i++){
            transposedGraph.addNode();
        }
        for(int row =0; row <costs.length;row++){
           for (int col =0; col < costs[row].length;col++){
               //System.out.println(row);
               //System.out.println(col);
               double cost =costs[col][row];
               transposedGraph.addEdge(row,col,cost);
               transposedGraph.getCosts()[row][col] =costs[col][row];
               transposedGraph.getCosts()[col][row] =costs[col][row];
           }
        }
